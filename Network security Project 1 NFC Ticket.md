...
In order to issue tickets and/or format new cards, we use the `use() ` function available in the file `/app/src/main/java/com/ticketapp/auth/ticket/Ticket.java` . The algorithm works as follows:

1. We first authenticate the key using the function `utils.authenticate()` method that was provided in the scaffolding code. If the authentication fails, `use()` fails.
2. We now read the card to get the data and parse it in the same way as `issue()` method.
3. We now check the app tag to make sure that it is the correct one.
4. We now check the version tag to make sure that it is the correct one.
5. We now check the card UID to make sure it exists.
6. We now generate the key, compute the MAC and check it in the same way as in `issue()` function.
7. We now check that the timestamp of first use and the validity period.
   1. If the time is expired (i.e. the current time is higher then the timestamp of first use plus the validity period), the method aborts and validates no ticket.
   2. If instead the the timestamp of first use is not set, we are using the card for the first time, so we set the timestamp of first use and of last use to current time and we increment the card counter by 1. We finally write everything to the card.
   3. We check that the last time the ticket has been used was less than a minute ago, since we don't want to use two tickets in case of a human error.
   4. If the timestamp of first use is set, then we increase the card counter by 1, update the last use timestamp and the logs that keep track of the last times the ticket was used and finally overwrite the card.
